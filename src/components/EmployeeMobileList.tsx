...
const EmployeeMobileCard = memo(({ 
  ...
  }, [isSelectionMode, onSelect, onTap, employee]);
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-3">
        ...
        <div className="flex-1 min-w-0 pr-8">
          <h3 className="text-base font-bold text-gray-900 dark:text-white truncate">{employee.name}</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 capitalize truncate">{employee.role}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-2.5 text-center">
          <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-0.5">Salary</p>
          <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
            ₹{employee.salary.toLocaleString()}
          </p>
        </div>
        <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-2.5 text-center">
          <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-0.5">Joined</p>
          <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
      ...
      <div className="flex items-center justify-between">
         <span className={cn(
            ...
            employee.kycStatus === 'verified' ? "bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400" :
            employee.kycStatus === 'pending' ? "bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400" :
...
EmployeeMobileCard.displayName = 'EmployeeMobileCard';

export const EmployeeMobileList: React.FC<EmployeeMobileListProps> = ({
  employees, onTap, onEdit, onSalary, onDelete, onBulkDelete
}) => {
  ...
  const getSelectedEmployees = () => employees.filter(e => selectedIds.has(e.id));
  const firstSelected = getSelectedEmployees()[0];
        ...
        {isSelectionMode && (
          ...
          >
            ...
            <div className="flex overflow-x-auto gap-2 p-3 snap-x hide-scrollbar mask-edges">
              {selectedIds.size === 1 ? (
                <>
                  <ActionButton icon={ClipboardList} label="Manage Tasks" primary onClick={() => { clearSelection(); onTap(firstSelected); }} />
                  <ActionButton icon={Edit2} label="Edit Info" onClick={() => { clearSelection(); onEdit(firstSelected); }} />
                  <ActionButton icon={DollarSign} label="Pay Salary" onClick={() => { clearSelection(); onSalary(firstSelected); }} />
