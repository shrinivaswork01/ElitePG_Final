        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-white/3 rounded-2xl p-4 grid grid-cols-2 gap-4">
                <div className="col-span-2 flex justify-between items-center">
                  ...
                  {canEdit && tenant.status === 'active' && (
                    <button
                      onClick={() => {
                        setSelectedRoomId(tenant?.roomId || tenant?.room_id || '');
                        setSelectedBedNumber(tenant?.bedNumber || tenant?.bed_number || null);
                        setRentOption('new');
                        setCustomRent(tenant?.rentAmount || tenant?.rent_amount || 0);
                        setSwitchSearchTerm('');
                        setSwitchDate(format(new Date(), 'yyyy-MM-dd'));
                ...
                <Field 
                  ...
                  value={
                    (() => {
                      const rNum = tenant.rooms?.room_number 
                        ? tenant.rooms.room_number 
                        : (tenant.room_number 
                          ? tenant.room_number 
                          : ( (tenant.roomId || tenant.room_id) 
                            ? rooms.find(r => r.id === (tenant.roomId || tenant.room_id))?.roomNumber 
                            : null ));
                      if (!rNum) return '—';
                ...
                <Field 
                  ...
                  value={
                    (() => {
                      const depAmt = tenant.depositAmount ?? tenant.deposit_amount ?? 0;
                      if (!depAmt) return '—';
                      
                      const depBal = tenant.depositBalance ?? tenant.deposit_balance ?? 0;
                      let status = tenant.depositStatus || tenant.deposit_status || 'Pending';
                      
                      if (depBal === 0 && depAmt > 0) {
                        status = 'Refunded';
                      }
                      
                      // Capitalize status
                      const formattedStatus = status.charAt(0).toUpperCase() + status.slice(1);
                      return `₹${Number(depAmt).toLocaleString()} (${formattedStatus})`;
                    })()
                  } 
                />
                ...
                <Field 
                  ...
                  value={
                    (() => {
                      const depAmt = tenant.depositAmount ?? tenant.deposit_amount ?? 0;
                      const depBal = tenant.depositBalance ?? tenant.deposit_balance ?? 0;
                      const adjusted = Math.max(0, depAmt - depBal);
                      
                      return (
                        <div className="flex flex-col items-start">
                          <span className={cn("font-bold", depBal > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-gray-500")}>
                            ₹{Number(depBal).toLocaleString()}
                          </span>
                          {adjusted > 0 && (
                            <span className="text-[10px] text-amber-600 dark:text-amber-500 font-medium mt-0.5">
                              (₹{adjusted.toLocaleString()} adjusted)
                            </span>
                          )}
                        </div>
                      );
              ...
              {!(tenant.roomSwitchDate || tenant.room_switch_date) &&
               !payments.some((p: any) => p.tenantId === tenant.id && p.paymentType === 'rent' && p.status === 'paid') &&
               (tenant.moveInDate || tenant.move_in_date) &&
               (tenant.tokenAmount ?? tenant.token_amount ?? 0) > 0 &&
               (tenant.tokenStatus || tenant.token_status) === 'paid' && (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-white/5 border border-gray-105 dark:border-white/5 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 text-gray-900 dark:text-white"
                      />
                    </div>
                  ...
                  {selectedRoomId && selectedBedNumber && (
                    <div className="space-y-3 animate-fade-in">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        >
                          Keep Current
                          <span className="block text-[10px] font-medium mt-0.5 opacity-80">₹{Number(tenant.rentAmount || tenant.rent_amount).toLocaleString()}/mo</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => {
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        let finalRent = tenant.rentAmount || tenant.rent_amount;
                        if (rentOption === 'new') {
                          const targetRoom = rooms.find(r => r.id === selectedRoomId);
                          finalRent = targetRoom?.price ?? (tenant.rentAmount || tenant.rent_amount);
